...
public interface Context {
	...
	/**
	 * Removes the mapping for a key from this context if it is present (optional operation).
	 * More formally, if this context contains a mapping from key <tt>k</tt> to value <tt>v</tt>
	 * such that {@code (key == null ? k == null : key.equals(k))}, that mapping is removed.
	 * (The context can contain at most one such mapping.)
	 *
	 ...
	 *
	 * <p>If this context permits null values, then a return value of <tt>null</tt> does not
	 * <i>necessarily</i> indicate that the context contained no mapping for the key;
	 * it's also possible that the context explicitly mapped the key to <tt>null</tt>.
	 *
	 * <p>The context will not contain a mapping for the specified key once the call returns.
	 *
	 ...
	 *
	 * <p>If you don't want to get the Context, you can use the {@link AsyncContext#importToCurrent()} proxy call
	 * <p>The Cleaner must be close after business:
	 * example:
	 * <pre>{@code
	 ...
	 * It will pass multiple key:value values required by Trace and Agent though
	 ...
	 *
	 * <p>When you want to call the next Server, you can pass the necessary key:value to the next Server
	 ...
	 *
	 * <p>It is usually called on the client request when collaboration between multiple server is required.
	 ...
	 *
	 * <p>It will not only obtain the key:value required by Trace from the {@link Request#header(String)},
	 * but also other necessary key:value of Agent, such as the key configured in the configuration file:
	 * {@link ProgressFields#AGENG_PROGRESS_FORWARDED_HEADERS_CONFIG}
	 *
	 * <p>If there is no Tracing Header, it will create a Root Span
	 *
	 ...
	 *
	 * <p>It is usually called on the server receives a request when collaboration between multiple server is required.
	 ...
	 * and {@link Request#cacheScope()}.
	 *
	 * <p>It will set the Span's tags "messaging.operation", "messaging.channel_kind" and "messaging.channel_name" from request
	 * {@link MessagingRequest#operation()} {@link MessagingRequest#channelKind()} {@link MessagingRequest#channelName()}
	 *
	 * <p>It is usually called on the consumer.
	 ...
	 * {@link MessagingRequest#operation()} {@link MessagingRequest#channelKind()} {@link MessagingRequest#channelName()}
	 *
	 * <p>It will not only pass multiple key:value values required by Trace through {@link Request#setHeader(String, String)},
	 * but also other necessary key:value of Agent, such as the key configured in the configuration file:
	 * {@link ProgressFields#AGENT_PROGRESS_FORAWRDED_HEADERS_CONFIG}
	 *
	 * <p>It is usually called on the producer.
	...
	/**
	 * Inject Producer's Span key:value and Forwarded Headers to Request {@link MessagingRequest#setHeader(String, String)}.
	 *
	 * @param span key:value from
	 * @param request key:value to
	 */
	...
	/**
	 * Returns a new child span if there's a {@link Tracing#currentSpan()} or a new trace of there isn't.
	 *
	 * @return {@link Span}
	 */
	Span nextSpan();
	...
	/**
	 * @return true if the key is necessary for Agent
	 */
	 ...
	 */
	Cleaner importForwardHeaders(Getter getter);
}
